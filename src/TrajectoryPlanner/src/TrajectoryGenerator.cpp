...
bool TrajectoryGenerator::configurePlanner(const yarp::os::Searchable& config)
{
    ...
    m_maxStepLength = config.check("maxStepLength", yarp::os::Value(1.1)).asFloat64();
...
void TrajectoryGenerator::computeThread()
{
    while (true)
    {
        ...
        else if (m_navigationConfig == NavigationSetup::ManualMode)
        {
            double s_theta = std::sin(unicycleAngle);
            double c_theta = std::cos(unicycleAngle);
...
{
    {
        ...
        m_personFollowingDesiredPath.zero();
        ...
        case NavigationSetup::NavigationMode:
            ...
            else if (m_unicycleController == UnicycleController::DIRECT)    //Use path of 3D poses (x, y, theta)
            {
                ...
                //Overwriting to zero to avoid conflicts
                m_desiredDirectControl(0) = 0.0;
